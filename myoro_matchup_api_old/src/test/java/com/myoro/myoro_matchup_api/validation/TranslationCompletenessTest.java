...
/** Test to verify that all translation keys from messages.properties exist in all locale files. */
class TranslationCompletenessTest {
  ...
  void testAllTranslationsPresent() throws IOException {
    ...
    Set<String> baseKeys =
        baseMessages.stringPropertyNames().stream()
    ...
    for (String localeFile : LOCALE_FILES) {
      ...
      Set<String> localeKeys =
          localeMessages.stringPropertyNames().stream()
              .filter(key -> !key.startsWith("#"))
              .collect(Collectors.toSet());
      ...
      if (!missing.isEmpty()) {
        missingTranslations.add(
            String.format(
                ...
                localeFile, missing.size(), String.join(", ", missing)));
      }
    }
