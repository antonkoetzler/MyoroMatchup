...
public class UserService {
  ...
  private UserSportStatsResponseDto buildSportStatsDto(UserModel user) {
    ...
    for (UserStatsModel stat : statsList) {
      if (stat.getSport() == SportsEnum.FOOTBALL && stat instanceof FootballUserStatsModel) {
        ...
        statsDto.setFootball(
            new FootballStatsDto(
                footballStat.getRating(),
                footballStat.getAttack(),
                footballStat.getDefense(),
                footballStat.getStriking(),
                footballStat.getSkills()));
        ...
        statsDto.setFutsal(
            new FootballStatsDto(
                futsalStat.getRating(),
                futsalStat.getAttack(),
                futsalStat.getDefense(),
                futsalStat.getStriking(),
                futsalStat.getSkills()));
        ...
        statsDto.setFut7(
            new FootballStatsDto(
                ...
                fut7Stat.getAttack(),
                fut7Stat.getDefense(),
                fut7Stat.getStriking(),
                fut7Stat.getSkills()));
      } else if (stat.getSport() == SportsEnum.VOLLEYBALL
          && stat instanceof VolleyballUserStatsModel) {
        VolleyballUserStatsModel volleyballStat = (VolleyballUserStatsModel) stat;
        statsDto.setVolleyball(
            new VolleyballStatsDto(
                volleyballStat.getRating(),
                volleyballStat.getAttack(),
                volleyballStat.getBlocking(),
                volleyballStat.getServing(),
                volleyballStat.getReception()));
      }
    }
  ...
  public void sendFriendRequest(final HttpServletRequest request, final Long recipientId) {
    ...
    if (recipient.getId().equals(requesterId)) {
      throw new RuntimeException(
          messageService.getMessage("error.friend.request.cannot.request.self"));
    }

    // Check if there's already a pending friend request
    if (friendRequestRepository.existsByRequesterIdAndRecipientIdAndStatus(
        requesterId, recipientId, FriendRequestStatusEnum.PENDING)) {
    ...
    if (friendRequestRepository.existsByRequesterIdAndRecipientIdAndStatus(
        recipientId, requesterId, FriendRequestStatusEnum.PENDING)) {
      throw new RuntimeException(messageService.getMessage("error.friend.request.already.pending"));
    }
  ...
  public List<UserResponseDto> getFriends(
      final HttpServletRequest request, final String query, final FriendRequestStatusEnum status) {
    final Long userId = jwtService.getUserIdFromRequest(request);
   ...
   * @param request the HTTP request containing the bearer token
   */
  public void deleteAccount(final HttpServletRequest request) {
    final Long userId = jwtService.getUserIdFromRequest(request);
    UserModel user = get(userId);

    // Delete related entities
    // Delete friend requests where user is requester or recipient
    friendRequestRepository.deleteAll(friendRequestRepository.findByRequesterId(userId));
    friendRequestRepository.deleteAll(friendRequestRepository.findByRecipientId(userId));

    // Delete blocked user relationships where user is blocker or blocked
    ...
    // Delete user stats
    userStatsRepository.deleteAll(userStatsRepository.findByUser(user));
