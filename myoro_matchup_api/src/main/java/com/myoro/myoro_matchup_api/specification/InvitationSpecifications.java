 ...
 * 
 * Provides reusable query specifications for invitation filtering.
 */
public class InvitationSpecifications {

  /**
   * Creates a specification to filter invitations by user (as invitee or
   * inviter).
   * 
   ...
   * - Expires date (formatted, if not null)
   * - Responded date (formatted, if not null)
   * 
  ...
  public static Specification<InvitationModel> search(String searchQuery) {
    ...
    return (root, criteriaQuery, criteriaBuilder) -> {
      ...
      predicates.add(criteriaBuilder.like(
          criteriaBuilder.lower(root.get("game").get("name").as(String.class)),
          searchTerm));

      // Search in inviter name
      predicates.add(criteriaBuilder.like(
          criteriaBuilder.lower(root.get("inviter").get("name").as(String.class)),
          searchTerm));
   ...
   */
  public static Specification<InvitationModel> filter(Long userId, String searchQuery,
      InvitationStatusEnum status) {
    return SpecificationHelper.and(
        forUser(userId),
        search(searchQuery),
        byStatus(status));
  }
